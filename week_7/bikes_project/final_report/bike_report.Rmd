...
* nominal e.g. bike id or start_station and end_station
...
1. Load the libraries and assign data to an object called *nyc_bikes_df*.
...
2. Dividing start time into 4 columns: *year, month, day and weekday.* 
...
4. Add *hire_length* column.

```{r}
nyc_bikes_df <- nyc_bikes_df %>% mutate(hire_length = as.period(stop_time - start_time))
```

5. Add *age* column and change to NA those who's age is over 126 (the highest age recorded so far in history is 123) and then drop them as there were only 2.
...
6. Divide *starting_time* to *hour*, *minute* and *second* columns.
...
9. Assign colour palette to an object called *colour_scheme*, so all visualisations are uniform.
